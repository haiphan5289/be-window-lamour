...
## Changelog — 2026-08-22: fix ô "Tên hàng" không hiện gợi ý + fallback kho ngầm định đổi từ "Kho chính" sang "HH"

> WPF-only, không đổi BE/schema. Phát hiện qua báo lỗi "gõ tên sản phẩm không thấy hiện list sản phẩm" ở popup "Phiếu nhập kho".

- **`WarehouseReceiptFormWindow.xaml` — cột "Tên hàng"**: cell editing template dùng `ComboBox` thường (`IsEditable="True"`) — không tự lọc theo text gõ, không tự mở dropdown khi gõ (chỉ nhảy tới item khớp gần nhất trong list hiện có, phải bấm mũi tên/F4 mới thấy list) — trong khi 2 field cạnh đó ("Đối tượng", "NV bán hàng") đã dùng `controls:AppSearchableComboBox` (có search-as-you-type) đúng chuẩn. Đổi sang `AppSearchableComboBox` (`ItemsSource="{Binding DataContext.Products,...}"`, `SelectedItem="{Binding SelectedProduct}"`, `IsNullable="True"`) — không cần sửa gì ở BE (`GET /api/v1/products` đã trả đủ list, WPF vẫn load-all-rồi-filter-client-side như các field khác).
- **Fallback kho ngầm định đổi `Id=1` ("Kho chính") → `Id=4` ("HH")**: `WarehouseReceiptFormViewModel.SaveAsync` — dòng `WarehouseId = (SelectedProduct as WarehouseProductItem)?.DefaultWarehouseId ?? 1` đổi thành `?? 4` — áp dụng khi sản phẩm chưa gán kho ngầm định (`Product.DefaultWarehouseId` là `int?`, có thể null). Cột "Kho" trên UI (hiển thị `SelectedProduct.DefaultWarehouseCode`) cũng thêm `TargetNullValue=HH` để khớp đúng giá trị sẽ được lưu, tránh hiển thị trống trong khi thực ra sẽ lưu là "HH". Warehouse `Id=4`/`Code="HH"`/`Name="Hàng hoá"` seed sẵn trong `WarehouseConfiguration.HasData` (cùng `Id=1` "Kho chính"/`Id=5` "TB" "Trưng bày").

---

---

## Changelog — 2026-08-28: fix "Bỏ ghi" (unconfirm) tô sai màu dòng trên "Nhập, Xuất Kho"

> User báo bug qua `/ct-be-to-desktop`: bấm "Bỏ ghi" 1 phiếu Nhập kho ở màn "Nhập, Xuất Kho" (`WarehouseTransactionListView`) xong, dòng đó không đổi màu/trạng thái như các dòng Treo khác.

`GetWarehouseTransactionsUseCase.MapReceipt` trước đó hardcode `IsHeld = false` cho mọi dòng Nhập kho — không đọc `WarehouseReceiptStatus` thật của `WarehouseReceipt`, nên dòng vừa "Bỏ ghi" (status quay về `Draft`) vẫn map ra `IsHeld = false` giống hệt dòng đã Confirmed, khiến WPF (tô màu/style theo `IsHeld`, đã có sẵn từ tính năng "Bỏ ghi" ở `WarehouseReceiptFormWindow` 2026-08-22) không phân biệt được 2 trạng thái. Fix: đổi thành `IsHeld = r.Status != WarehouseReceiptStatus.Confirmed` — bất kỳ status nào khác Confirmed (Draft sau khi tạo mới lẫn Draft sau khi Bỏ ghi) đều coi là "đang treo" cho mục đích hiển thị ở danh sách gộp này, khớp đúng ý nghĩa cột "IsHeld" đã dùng ở nơi khác trong hệ thống. Chỉ áp dụng cho nhánh Nhập kho (`MapReceipt`) — nhánh Xuất kho (Sales Order, `MapSalesOrder`) không đổi vì Sales Order không có khái niệm Draft/Confirmed riêng biệt trong luồng này. Không cần EF migration (không đổi schema, chỉ sửa 1 dòng map logic). BE build 0 lỗi.

---

*Generated by `/ct-ai-document` on 2026-04-27 | Updated 2026-04-28: LatestAccountingDate column + timezone bug fix + flat list layout (Mã hàng / Tên hàng) | Updated 2026-08-15: 4 loại phiếu + Supplier + tab Thống kê + số phiếu `NK00048` | Updated 2026-08-15 (×2): `GET /api/v1/warehouse-transactions` danh sách gộp Nhập/Xuất kho + đổi prefix Sales Order BC → XK | Updated 2026-08-22: fix "Tên hàng" không gợi ý (migrate sang `AppSearchableComboBox`) + fallback kho ngầm định "HH" | Updated 2026-08-28: fix "Bỏ ghi" không đổi màu dòng trên "Nhập, Xuất Kho" (`IsHeld` hardcode `false` → đọc `WarehouseReceiptStatus` thật)*
