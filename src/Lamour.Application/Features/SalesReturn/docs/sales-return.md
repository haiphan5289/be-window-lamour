...
## Update — 2026-09-14: đảo ngược 1 phần quyết định 09-11 — số PN cố định, không tăng khi sửa BTL

**Phản ánh của user**: đơn BTL00010 in ra NK00014; sửa sản phẩm rồi in lại → NK00015; sửa thêm 1 lần
nữa → NK00016. Kỳ vọng: số NK chỉ tăng khi tạo **BTL mới**, sửa 1 BTL đã có rồi in lại phải ra
**đúng số PN cũ**, không tạo số mới.

**Bối cảnh**: đây chính là hành vi đã cân nhắc kỹ và CHỌN có chủ đích ở bản 09-11 phía trên (đánh đổi
"số PN đổi mỗi lần sửa" để luôn in đúng dữ liệu hiện tại). Được hỏi lại và xác nhận: **đổi ưu tiên** —
giờ số PN cố định quan trọng hơn, chấp nhận đánh đổi ngược lại.

**Fix**: `CreateSalesReturnWarehouseReceiptUseCase.ExecuteAsync` — nhánh "lệch chữ ký dòng hàng"
không còn đánh dấu `IsSuperseded=true` + lập PN mới nữa. Thay vào đó **sửa thẳng** entity PN đang có
(`Lines`/`TotalAmount`/`AccountingDate`/`DocumentDate`/`Description`/`DeliveryPerson`) rồi
`SaveChangesAsync` — giữ nguyên `Id`/`ReceiptNumber`. An toàn làm trực tiếp vậy (không cần đi qua
`Unconfirm→Update→Reconfirm`) vì PN loại `ReturnedGoods` này vốn **không bao giờ đụng
`Product.StockQuantity`** (cộng/trừ kho thật nằm ở `CreateSalesReturnUseCase`/`UpdateSalesReturnUseCase`,
không phải ở PN này) — khác hẳn lo ngại đã nêu ở bản 09-11 (vốn chỉ áp dụng nếu đi qua
`UnconfirmWarehouseReceiptUseCase`, use case này giờ không đụng tới nữa.

`IsSuperseded`/cột `is_superseded` **giữ nguyên trong entity** (không rollback migration) — các bản
ghi đã bị supersede từ trước 09-14 vẫn còn để đối chiếu lịch sử, chỉ là từ giờ không có bản ghi mới
nào bị đánh dấu supersede nữa (nhánh "lệch" giờ update tại chỗ thay vì tạo mới).

WPF không cần đổi gì — `SalesReturnViewModel.PrintAsync` vẫn gọi thẳng
`_createWarehouseReceipt.ExecuteAsync` như bản 09-11, hành vi mới hoàn toàn nằm trong use case này.

**Kèm theo (cùng ngày, WPF thuần — `desktop-lamour`)**: địa chỉ công ty trên đầu phiếu in
"PHIẾU NHẬP KHO" ("Số 110/20/38 Đường số 30, Phường An Nhơn, TP Hồ Chí Minh.") bị xuống dòng do khổ
A5 hẹp + Floater logo chiếm chỗ ngang — giảm `FontSize` riêng dòng này (13→11) + bớt phần chỗ trống
thừa của Floater logo (135→124px) để nằm gọn 1 dòng. File: `WarehouseReceiptPrintWindow.xaml.cs`
(`Features/HomePage/Warehouse/Views/`). Chưa xác nhận bằng cách in/preview thật trên UTM.

**Review artifact đã cập nhật** (lần soát xét 4, 14/09): https://claude.ai/code/artifact/099e21eb-0bc7-4ee8-899e-7483f0093092
