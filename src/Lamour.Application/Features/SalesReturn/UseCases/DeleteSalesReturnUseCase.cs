...
public class DeleteSalesReturnUseCase : IDeleteSalesReturnUseCase
{
    ...
    public async Task ExecuteAsync(int id, CancellationToken ct = default)
    {
        ...
        try
        {
            // Chứng từ Confirmed thì đã cộng tồn kho lúc ghi sổ — xóa phải rút lại. Hàng trả lại =
            // nhập kho, nên hoàn tác = TRỪ tồn kho → kiểm tra đủ tồn trước (đã xuất bán tiếp sau khi
            // lưu thì không cho xóa), hai lượt như cũ. Chứng từ đang Draft (đã "Bỏ ghi" —
            // UnconfirmSalesReturnUseCase đã tự rút tồn kho rồi) thì KHÔNG được rút lại lần nữa.
