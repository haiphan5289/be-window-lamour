...
| Summary-report công thức | `SalesAmount` = Σ(Quantity×UnitPrice) gross **cho dòng thường**; `DiscountAmount` = Σ(Quantity×UnitPrice×DiscountRate/100) **cho dòng thường**; `ReturnValue` = Σ(SalesReturnLine.Amount − SalesReturnLine.DiscountAmount) (net); `NetRevenue` = SalesAmount − DiscountAmount − ReturnValue. "Giá trị giảm giá" (price-reduction riêng biệt với chiết khấu) KHÔNG được model trong domain — WPF hiển thị cột này nhưng luôn = 0 |
...
| Summary-report + dòng `IsAmountManual` (fix 2026-09-24) | Dòng "Cọc"/"Trừ cọc" (và mọi dòng Thành tiền thủ công khác) luôn có `Quantity = 0`, `UnitPrice = 0` — công thức `Quantity×UnitPrice` ở trên cho dòng thường sẽ luôn ra `0` với các dòng này dù `Amount` đã lưu khác 0 (dương với Cọc, âm với Trừ cọc), khiến sản phẩm vẫn xuất hiện trong report (có transaction trong kỳ) nhưng MỌI cột tiền = 0. Sửa: nếu `SalesOrderLine.IsAmountManual = true` → `SalesAmount += l.Amount` trực tiếp (bỏ qua Quantity×UnitPrice, không cộng riêng `DiscountAmount` vì `Amount` đã là số cuối). Áp dụng cho MỌI report type ở dropdown WPF (Mặt hàng, Khách hàng, Nhân viên, ...) vì tất cả dùng chung `GetSalesOrderSummaryReportUseCase` |
...
*Updated 2026-08-28 (thêm `POST /api/v1/sales-orders/{id}/duplicate` — "Nhân bản" chứng từ bán hàng): theo yêu cầu thêm menu chuột phải (Thêm/Nhân bản/Xem/Xóa/Gửi email,Zalo) trên `SalesOrderListView`. `IDuplicateSalesOrderUseCase`/`DuplicateSalesOrderUseCase` (feature `Sales`) — đọc chứng từ gốc qua `IGetSalesOrderByIdUseCase`, sinh số chứng từ mới qua `IGetNextSalesOrderCodeUseCase` (tái dùng, không viết lại logic sinh số), set `AccountingDate`/`DocumentDate` = hôm nay (KHÔNG copy ngày chứng từ gốc), copy nguyên `Lines` (product/quantity/giá/CK/thuế/TK...) qua `ICreateSalesOrderUseCase.ExecuteAsync` — tái dùng toàn bộ validate/side-effect (trừ kho, tính thuế...) của luồng tạo mới bình thường, không có code đường tắt riêng. Route mới trên `SalesOrdersController`: `POST {id}/duplicate` → 201 bản sao mới, 404 nếu id gốc không tồn tại. DI đăng ký `IDuplicateSalesOrderUseCase` trong `Program.cs`. Không cần EF migration (không đổi schema). WPF: `SalesOrderListViewModel.DuplicateSalesOrderCommand` gọi endpoint này rồi reload list; menu chuột phải thêm mới trên `SalesOrderListView` (Thêm/Nhân bản/Xem/Xóa/Gửi email,Zalo qua `PreviewMouseRightButtonDown` tự chọn dòng dưới con trỏ trước khi mở); cột "Diễn giải"/"Tiền thuế GTGT" thêm vào `SalesOrderListItem`/grid — xem `desktop-lamour/.../Sales/docs/sales.md` cho chi tiết context menu + cột mới thêm cùng đợt.*

## Update — 2026-09-01: Tồn kho chỉ phản ánh đơn Normal — Treo không còn trừ/giữ kho

Theo yêu cầu: *"Chứng từ bán hàng nếu đơn đang treo thì sản phẩm không trừ vào tồn kho, trừ khi đơn hoàn thành mới trừ vào tồn kho"*. Trước đây `CreateSalesOrderUseCase` trừ kho **vô điều kiện** ngay lúc Tạo (mọi đơn mới đều `Status = Normal` — không có khái niệm "Treo ngay từ lúc tạo" ở BE), và `HoldSalesOrderUseCase` chỉ đổi `Status` mà không đụng gì tới tồn kho — nghĩa là tồn kho **luôn** bị trừ ngay cả khi đơn sau đó bị Treo, hoàn toàn ngược với yêu cầu trên.

**Bất biến mới, áp dụng nhất quán ở cả 4 UseCase mutate:** `StockQuantity` chỉ phản ánh các đơn đang **Normal** ("Ghi sổ"/hoàn thành). Đơn đang **Held** ("Treo") không bao giờ có tác động tồn kho nào.

- **`HoldSalesOrderUseCase`** — thêm guard chặn Treo 1 đơn **đã Treo sẵn** (`DomainException`, trước đây có thể treo bất kể trạng thái). Vì order chỉ có thể tới được đây khi đang **Normal** (đã qua guard), tồn kho chắc chắn đã từng bị trừ — Hold giờ **hoàn lại** đúng số đó (two-pass: validate tồn hiện tại đủ để hoàn trước, rồi mới trừ thật — mirror `UnconfirmSalesReturnUseCase`/`UnconfirmWarehouseReceiptUseCase`) rồi mới set `Status = Held`. Thêm `IUnitOfWork` transaction (trước đây không có).
- **`UpdateSalesOrderUseCase`** ("💾 Ghi sổ") — luôn đưa đơn về `Normal` như cũ, nhưng bước **hoàn tồn kho cũ** giờ chỉ chạy nếu đơn **trước đó đã Normal** (`wasHeld = order.Status == Held`, chụp TRƯỚC khi ghi đè `Status`) — nếu đơn đang Treo, tồn kho cho các dòng cũ chưa từng bị trừ nên không có gì để hoàn. Bước **trừ tồn kho mới** vẫn LUÔN chạy vô điều kiện (Update luôn kết thúc ở Normal = hoàn thành, dù trước đó Normal hay Treo) — đây chính là lần đầu tiên tồn kho thật sự bị trừ nếu đơn trước đó đang Treo.
- **`DeleteSalesOrderUseCase`** — chỉ hoàn tồn kho nếu đơn đang **Normal** lúc xóa (thêm điều kiện `order.Status == SalesOrderStatus.Normal`) — xóa 1 đơn đang Treo không cần hoàn tác gì.
